...
class BertLSTMEncoder(nn.Module):
    ...
    def forward(self, src):
        ...
        mask_local = ~(src.data.eq(self.padding_idx).view(batch_size * n_paras, n_tokens).bool())
        src_len = torch.sum(mask_local, -1).long()
        src = src.view(batch_size * n_paras, -1)
        ...
        enc_art, final_states = lstm_encoder(bert_hidden, self.enc_lstm, src_len,
                                             init_enc_states, None, {}, {})
        enc_art = self.projection(enc_art)
        enc_art = enc_art.transpose(0, 1).contiguous()
        # print(src.size(), enc_art.size(), mask_local.size())
        para_vec = self.pooling(enc_art, enc_art, ~mask_local)
        para_vec = para_vec.view(-1, self.d_per_head)
        para_vec = self.layer_norm1(para_vec)
        para_vec = self.dropout1(para_vec).view(batch_size * n_paras, 1, -1)
        src_features = enc_art + para_vec
        src_features = self.feed_forward(src_features.view(-1, self.n_hidden))
        src_features = src_features.view(batch_size * n_paras, n_tokens, -1)
        para_vec = para_vec.view(batch_size, n_paras, -1)
        mask_hier = mask_local[:, :, None].float()
        src_features = src_features * mask_hier
        src_features = src_features.view(batch_size, n_paras, n_tokens, -1)
